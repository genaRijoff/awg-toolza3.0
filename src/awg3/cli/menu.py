...
class Menu:
    ...
    def _apply_to_interface(self) -> bool:
        ...
        if not self.backend.is_running():
            ...
            info("подними его: «Сервер» → 3 «Запустить интерфейс»")
            return False
    ...
    def create_server(self) -> None:
        ...
        self._auto_start()
        info("дальше: «Клиенты» → 1 «Добавить клиента»")
    ...
    def regenerate_obfuscation(self) -> None:
        ...
        info("выдай каждому новый: «Клиенты» → 3 «Показать конфиг»")
    ...
    def _auto_start(self) -> None:
        ...
        остановил туннель («Сервер» → 4), добавление клиента не должно его будить.
        """
        print()
        try:
            for line in actions.bring_up(self.storage):
    ...
    def show_status(self) -> None:
        ...
        if hpk:
            ...
            if low:
                ...
                print(f"  {D}{'':<18}{N} {Y}перегенерируй: «Сервер» → 6{N}")
            else:
    ...
    def backup_menu(self) -> None:
        ...
        self.storage = Storage()
        self.backend = GoBackend(iface=self._iface())
        info("применить: «Сервер» → 3 «Запустить интерфейс»")
